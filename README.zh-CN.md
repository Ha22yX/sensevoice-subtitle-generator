...
**导入视频 → AI 自动生成带时间轴字幕(SRT / VTT),并独家支持 ♿ 无障碍 SDH 字幕(声音与情感标注)与烧录到视频。**

[English](README.md) · [中文](README.zh-CN.md)

基于 [SenseVoice](https://github.com/FunAudioLLM/SenseVoice) 多语种语音识别,经 [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx) 在**本地**推理 ——
无需联网、无需 API Key,中 / 英 / 日 / 韩 / 粤等 50+ 语言自动识别。
...
- 📝 **标准格式**:SRT + WebVTT,兼容几乎所有播放器。
- ♿ **无障碍 SDH 字幕(独家)**:利用 SenseVoice 的**情感识别 + 音频事件检测**,生成带
  `[掌声]` `[笑声]` `[背景音乐]` 等声音标注、并按情感着色(ASS)的字幕 ——
  听障人士、内容创作者、语言学习者的刚需,**目前没有同类开源工具**。
- 🔥 **烧录字幕到视频**:可把带样式的字幕(含 SDH 情感色)直接渲染进 MP4,即得可分享的视频。
- 🧩 **零配置依赖**:ffmpeg 由 `imageio-ffmpeg` 自带,无需单独安装。
- 💻 **跨平台**:Windows / macOS / Linux,默认纯 CPU 即可运行;可选 GPU。

## 🖼️ 截图

![界面](docs/ui.png)

![SDH 字幕烧录效果(情感着色 + 声音标注)](docs/sdh_demo.png)

> 上图为烧录到视频后的实际效果:开心(HAPPY)台词显示为**橙色**,并带有 `[背景音乐]` 声音标注。
...
视频 ──(ffmpeg 抽 16kHz 单声道 PCM)──► silero-vad 语音分段
        └── 每段 ► SenseVoice 解码 ► 文本 + 语种 + 情感 + 事件 + 段级时间戳
                                          │
                            ┌─────────────┴─────────────┐
                            ▼                           ▼
                       SRT / VTT                SDH-SRT / 情感着色 ASS
                                                        │
                                              (可选)烧录进 MP4
```
...
| **烧录字幕到视频** | 把生成的字幕直接渲染进一个 MP4(较慢,需重新编码)。SDH 模式下情感着色一并保留。 |
...
│   ├── subtitles.py       # SRT / VTT 格式化 + 片段整理
│   ├── sdh.py             # 无障碍 SDH(SRT + ASS,声音/情感标注)
│   └── burn.py            # 字幕烧录进视频(libass)
├── docs/                  # 截图
...
- **[SenseVoice](https://github.com/FunAudioLLM/SenseVoice)**(FunAudioLLM)—— 多语种语音识别 + 情感 + 事件检测
